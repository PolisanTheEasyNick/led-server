...
void
remove_client_fd(int client_fd)
{
    pthread_mutex_lock(&clients_mutex);
    ...
    if (index != -1)
    {
        ...
        if (new_clients_fds
            || clients_count == 0)  // realloc could return NULL if size is 0
        {
            clients_fds = new_clients_fds;
        }
    }
                ...
                switch (result.version)
                {
                    ...
                    case 3:
                    {
                        ...
                        switch (result.OP)
                        {
                            ...
                            {
                                logger(TCP,
                                       "Requested LED_SET_COLOR with %d %d %d on %d seconds, setting.",
                                       result.RED,
                                       result.GREEN,
                                       result.BLUE,
                                       result.duration);
                                set_color_duration(pi,
                                                   (struct Color) {result.RED, result.GREEN, result.BLUE},
                                                   result.duration);
                                break;
                            }
