...
void
openrgb_init_header(uint8_t *header, uint32_t pkt_dev_idx, uint32_t pkt_id, uint32_t pkg_size)
{
    // adding magick
    header[0] = 'O';
    header[1] = 'R';
...
void
openrgb_request_controller_data(uint32_t pkt_dev_idx)
{
...
void
openrgb_request_update_leds(uint32_t pkt_dev_idx, struct Color color)
{
...
void
openrgb_set_color_on_devices(struct Color color)
{
    ...
    while (!openrgb_stop_server)
    {
        ...
        switch (pkt_id)
        {
            case OPENRGB_NET_PACKET_ID_REQUEST_CONTROLLER_DATA:
            {
                ...
                {
                    tmp_bytes_received = recv(openrgb_socket,
                                              response + bytes_received,
                                              pkt_size - bytes_received,
                                              0);

                    if (tmp_bytes_received <= 0)
                    {
                        logger_debug(OPENRGB,
                                     "Error: Failed to receive data. Received %d bytes so far.",
                                     bytes_received);
            ...
            case OPENRGB_NET_PACKET_ID_REQUEST_PROTOCOL_VERSION:
            {
                ...
                openrgb_using_version =
                    openrgb_version <= OPENRGB_SUPPORTED_VERSION
                    ? openrgb_version
                    : OPENRGB_SUPPORTED_VERSION;
                logger_debug(OPENRGB,
...
void *
openrgb_reconnect_thread(void *arg)
{
